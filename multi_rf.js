...
Uart_info_file = 'drone.json';

try {
    drone = JSON.parse(fs.readFileSync(Uart_info_file, 'utf8'));
}
catch (e) {
    var info = {};
    ...
    fs.writeFileSync(Uart_info_file, JSON.stringify(drone, null, 4), 'utf8');
}
...
for (var idx in drone) {
    if (drone.hasOwnProperty(idx)) {
        drone[idx].parent_topic = [];
        drone[idx].cnt_topic = [];

        var parent = '/Mobius/' + ae_name + '/Drone_Data/' + drone[idx].name;
        drone[idx].parent_topic.push(parent);
        var cnt = '/Mobius/' + ae_name + '/Drone_Data/' + drone[idx].name + '/disarm';
        drone[idx].cnt_topic.push(cnt);
    }
}

for (var idx in drone) {
    if (drone.hasOwnProperty(idx)) {
        drone[idx].sub_gcs_topic = [];
        drone[idx].noti_topic = [];

        var sub_gcs = '/Mobius/' + ae_name + '/GCS_Data/' + drone[idx].name;
        drone[idx].sub_gcs_topic.push(sub_gcs);
        var noti = util.format('/oneM2M/req/+/S%s/#', ae_name);
        drone[idx].noti_topic.push(noti);
    }
}
...
function tas_ready() {
    ...
    for (var idx in drone){
        if (drone.hasOwnProperty(idx)) {
            ...
            var port = null;
            port = mavPortOpening(port, drone[idx].portNum);
        }
        mavPort.push(port);
    }
    drone.push(mavPort);
...
function mavPortOpening(serialPort, mavPortNum) {
    console.log('mavPort ' + mavPortNum + ' Opening...');
    if (serialPort == null) {
        serialPort = new SerialPort(mavPortNum, {
            baudRate: parseInt(57600, 10),
        });

        serialPort.on('open', mavPortOpen);
        serialPort.on('close', mavPortClose);
        serialPort.on('error', mavPortError);
        serialPort.on('data', mavPortData);
    }
    else {
        if (serialPort.isOpen) {
...
function mavPortOpen() {
    console.log('mavPort open. ');
    // console.log('mavPort open. ' + mavPortNum + ' Data rate: 57600');
//    mqtt_connect('localhost', 1883, sub_gcs_topic, noti_topic);
}

function mavPortClose(serialPort, mavPortNum) {
...
//                    mqtt_client.publish(my_cnt_name, Buffer.from(mavPacket, 'hex'));
                    for (var idx in drone) {
                        if (drone.hasOwnProperty(idx)) {
                            if (parseInt(sysid, 16) == drone[idx].system_id) {
                                var cnt_name = drone[idx].cnt_topic;
                                mqtt_client.publish(cnt_name, Buffer.from(mavPacket, 'hex'));
                            }
                        }
                    }
...
function parseMav(mavPacket) {
    ...
    console.log('==============================================');
    console.log('SYSTEM ID: ', parseInt(Number('0x'+sysid), 10));
    console.log('Data: ', mavPacket);
    console.log('==============================================');
        ...
        if(fc.heartbeat.base_mode & 0x80) {
            if(flag_base_mode == 0) {
                ...
                for (var idx in drone) {
                    if (drone.hasOwnProperty(idx)) {
                        if (parseInt(sysid, 16) == drone[idx].system_id) {
                            var my_cnt_name = drone[idx].parent_topic + '/' + my_sortie_name;
                        }
                    }
                }
